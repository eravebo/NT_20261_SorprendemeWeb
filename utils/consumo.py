...
def consumir_pedidos():
    """
    Llama al endpoint GET /api/pedidos del backend Spring Boot,
    convierte la respuesta JSON en un DataFrame de pandas y lo retorna.

    Columnas esperadas: id, nombre_cliente, telefono, email,
                        direccion_envio, fecha, estado, mp_payment_id, mp_status
    """
    ...
    try:
        ...
        print(f"ERROR HTTP: {error_http}")
        print("Verifica que el endpoint /api/pedidos exista en el backend.")
        return None
...
def consumir_pedidos_desde_json(ruta_archivo="BD_Simulacion/pedidos_dataset.json"):
    """
    Alternativa local: carga los datos desde un archivo JSON en disco.
    Úsala cuando el backend no esté disponible o para pruebas rápidas.
    """
    try:
        ...
        print(f"ERROR: No se encontró el archivo: {ruta_archivo}")
        print("Primero corre generar_simulacion_pedido() para crear los datos de prueba.")
        return None
